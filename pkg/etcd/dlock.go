...
func (etcd *Etcd) Dlock(ctx context.Context, key string, cb callback) error {
	logger.Debugf(ctx, "Create dlock with key [%s]", key)
	mutex, err := etcd.NewMutex(key)
	if err != nil {
		logger.Criticalf(ctx, "Dlock lock error, failed to create mutex: %+v", err)
		return err
	}
	err = mutex.Lock(ctx)
	if err != nil {
		logger.Criticalf(ctx, "Dlock lock error, failed to lock mutex: %+v", err)
		return err
	}
