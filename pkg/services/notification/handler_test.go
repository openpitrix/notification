...
func TestValidateEmailService(t *testing.T) {
	...
	emailcfg := &pb.EmailServiceConfig{
		...
		Email:           pbutil.ToProtoString("openpitrix@foxmail.com"), //stmp user
...
func TestValidateEmailServiceV2(t *testing.T) {
	if !*pkg.LocalDevEnvEnabled {
		t.Skip("Local Dev testing env disabled.")
	}
	...
	var req = &pb.ValidateEmailServiceV2Request{
		TestEmailRecipient: pbutil.ToProtoString("*****@qq.com"),
		EmailServiceConfig: emailcfg,
	}

	resp, err := s.ValidateEmailServiceV2(ctx, req)
	if err != nil {
		t.Fatalf("Test ValidateEmailService failed")
	}
