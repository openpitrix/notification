...
import (
	...
	notification "notification/pkg/pb"
	nfmock "notification/pkg/services/mock/mockgen"
)
...
func TestCreateNfWaddrs(t *testing.T) {
	...
	mockNotificationClient := nfmock.NewMockNotificationClient(ctrl)
	req := &notification.HelloRequest{Name: "unit_test"}
	//req:=&notification.CreateNfResponse{}
	mockNotificationClient.EXPECT().SayHello(
		gomock.Any(),
		&rpcMsg{msg: req},
	).Return(&notification.HelloReply{Message: "Mocked Interface"}, nil)
	testCreateNfWaddrs(t, mockNotificationClient)
}

func testCreateNfWaddrs(t *testing.T, client notification.NotificationClient) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := client.SayHello(ctx, &notification.HelloRequest{Name: "unit_test"})
	t.Log("Test function : SayHello")
