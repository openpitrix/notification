...
func TestNew(t *testing.T) {
	var e error
	e = New(ctx, Internal, ErrorGetServiceConfigFailed)
	assert.Equal(t, e.Error(), "rpc error: code = Internal desc = 获取通知服务参数配置失败")
...
func TestNewWithDetail(t *testing.T) {
	var e error
	e = NewWithDetail(ctx, InvalidArgument, fmt.Errorf("test with error detail"), ErrorGetServiceConfigFailed)
	assert.Equal(t, e.Error(), "rpc error: code = InvalidArgument desc = 获取通知服务参数配置失败: test with error detail")
	ge := status.Convert(e)
	assert.Equal(t, ge.Code().String(), "InvalidArgument")
	assert.Equal(t, ge.Err().Error(), "rpc error: code = InvalidArgument desc = 获取通知服务参数配置失败: test with error detail")
	...
	e = NewWithDetail(ctx, InvalidArgument, errors.New("test with error detail"), ErrorGetServiceConfigFailed)
	ge = status.Convert(e)
